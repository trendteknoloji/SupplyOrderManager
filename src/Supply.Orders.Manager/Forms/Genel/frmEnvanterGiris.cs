...
{
    public partial class frmEnvanterGiris : frmBase
    {
        SOMContext mainContext = new SOMContext();
        public frmEnvanterGiris()
        {
            InitializeComponent();
        }
        ...
        private void gluSCins_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                if (gluSCins.EditValue != null && gluSKalite.EditValue != null)
                {
                    ...
                    //txtStokKodu.Text = string.Format("{0}{1}{2}", cinsArtikel, kaliteArtikel, stok_sira);
        ...
        private void btnSatirEkle_Click(object sender, EventArgs e)
        {
            ...
            {
                List<mdlEnvanter> EnvanterListe = new List<mdlEnvanter>();
                if (gcSiparis.DataSource != null)
                {
                    try
                    {
                        EnvanterListe = (List<mdlEnvanter>)gcSiparis.DataSource;
                    }
                    catch (Exception Ex)
                    {
                        ExceptionHelpers.ErrorRegister(Ex);
                ...
                try
                {
                    ...
                    EnvanterListe.Add(new mdlEnvanter
                    {
                    ...
                    gcSiparis.DataSource = EnvanterListe;
                ...
                {
                    ExceptionHelpers.ErrorRegister(Ex);
                    DialogResult dr = XtraMessageBox.Show("Bu stok kodu daha önce eklendiği için tekrar eklenemez lütfen yeni stok kodu giriniz.\nYada HAYIR seçeneği ile devam edebilirsiniz.", "Eksik Bilgi", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Error);
        ...
        private void btnEvrakKaydet_Click(object sender, EventArgs e)
        {
            if (gcSiparis.DataSource != null)
            {
                List<mdlEnvanter> EnvanterListe = (List<mdlEnvanter>)gcSiparis.DataSource;
                mainContext.Envanter.AddRange(EnvanterListe);
